...
typedef struct player_engine_vgm PE_VGM;
...
typedef struct vgm_pcm_stream_device
{
	DEV_INFO defInf;
	UINT8 streamID;
	UINT8 bankID;
	...
	UINT32 lastItem;
	UINT32 maxItems;
} VGM_PCMSTRM_DEV;	// DACSTRM_DEV
typedef struct vgm_play_options
{
...
UINT32 VGMEngine_GetModifiedLoopCount(const PE_VGM* self, UINT32 defaultLoops);	// get loop count, modified according to LoopModified/LoopBase header
...
{
public:
OK	struct DEVLOG_CB_DATA
	{
		VGMPlayer* player;
		size_t chipDevID;
	};
OK	struct CHIP_DEVICE	// Note: has to be a POD, because I use memset() on it.
	{
		VGM_BASEDEV base;
		UINT8 vgmChipType;
		DEV_ID chipType;
...
protected:
OK	struct XHDR_DATA32
	{
		UINT8 type;
		UINT32 data;
	};
OK	struct XHDR_DATA16
	{
		UINT8 type;
		UINT8 flags;
		UINT16 data;
...
OK	typedef void (VGMPlayer::*COMMAND_FUNC)(void);	// VGM command member function callback
OK	struct DEVLINK_CB_DATA
	{
		VGMPlayer* player;
		SONG_DEV_CFG* sdCfg;
		CHIP_DEVICE* chipDev;
	};
OK	struct COMMAND_INFO
	{
		UINT8 chipType;
		UINT32 cmdLen;
		COMMAND_FUNC func;
...
public:
	...
	UINT32 VGMEngine_GetPlayerType(PE_VGM* self, void) const;
	const char* VGMEngine_GetPlayerName(PE_VGM* self, void) const;
	static UINT8 VGMEngine_PlayerCanLoadFile(PE_VGM* self, DATA_LOADER *dataLoader);
	UINT8 VGMEngine_CanLoadFile(PE_VGM* self, DATA_LOADER *dataLoader) const;
	UINT8 VGMEngine_LoadFile(PE_VGM* self, DATA_LOADER *dataLoader);
	UINT8 VGMEngine_UnloadFile(PE_VGM* self, void);
	const VGM_HEADER* VGMEngine_GetFileHeader(PE_VGM* self, void) const;
	
	const char* const* VGMEngine_GetTags(PE_VGM* self, void);
	UINT8 VGMEngine_GetSongInfo(PE_VGM* self, PLR_SONG_INFO& songInf);
	UINT8 VGMEngine_GetSongDeviceInfo(PE_VGM* self, std::vector<PLR_DEV_INFO>& devInfList) const;
	UINT8 VGMEngine_SetDeviceOptions(PE_VGM* self, UINT32 id, const PLR_DEV_OPTS& devOpts);
	UINT8 VGMEngine_GetDeviceOptions(PE_VGM* self, UINT32 id, PLR_DEV_OPTS& devOpts) const;
	UINT8 VGMEngine_SetDeviceMuting(PE_VGM* self, UINT32 id, const PLR_MUTE_OPTS& muteOpts);
	UINT8 VGMEngine_GetDeviceMuting(PE_VGM* self, UINT32 id, PLR_MUTE_OPTS& muteOpts) const;
	// player-specific options
	UINT8 VGMEngine_SetPlayerOptions(PE_VGM* self, const VGM_PLAY_OPTIONS& playOpts);
	UINT8 VGMEngine_GetPlayerOptions(PE_VGM* self, VGM_PLAY_OPTIONS& playOpts) const;
	
	//UINT32 VGMEngine_GetSampleRate(PE_VGM* self, void) const;
	UINT8 VGMEngine_SetSampleRate(PE_VGM* self, UINT32 sampleRate);
	double VGMEngine_GetPlaybackSpeed(PE_VGM* self, void) const;
	UINT8 VGMEngine_SetPlaybackSpeed(PE_VGM* self, double speed);
	//void VGMEngine_SetEventCallback(PE_VGM* self, PLAYER_EVENT_CB cbFunc, void* cbParam);
	//void VGMEngine_SetFileReqCallback(PE_VGM* self, PLAYER_FILEREQ_CB cbFunc, void* cbParam);
	UINT32 VGMEngine_Tick2Sample(PE_VGM* self, UINT32 ticks) const;
	UINT32 VGMEngine_Sample2Tick(PE_VGM* self, UINT32 samples) const;
	double VGMEngine_Tick2Second(PE_VGM* self, UINT32 ticks) const;
	//double VGMEngine_Sample2Second(PE_VGM* self, UINT32 samples) const;
	
	UINT8 VGMEngine_GetState(PE_VGM* self, void) const;
	UINT32 VGMEngine_GetCurPos(PE_VGM* self, UINT8 unit) const;
	UINT32 VGMEngine_GetCurLoop(PE_VGM* self, void) const;
	UINT32 VGMEngine_GetTotalTicks(PE_VGM* self, void) const;	// get time for playing once in ticks
	UINT32 VGMEngine_GetLoopTicks(PE_VGM* self, void) const;	// get time for one loop in ticks
	//UINT32 VGMEngine_GetTotalPlayTicks(PE_VGM* self, UINT32 numLoops) const;	// get time for playing + looping (without fading)
	
	UINT32 VGMEngine_GetModifiedLoopCount(PE_VGM* self, UINT32 defaultLoops) const;	// get loop count, modified according to LoopModified/LoopBase header
	const std::vector<DACSTRM_DEV>& VGMEngine_GetStreamDevInfo(PE_VGM* self, void) const;
	
	UINT8 VGMEngine_Start(PE_VGM* self, void);
	UINT8 VGMEngine_Stop(PE_VGM* self, void);
	UINT8 VGMEngine_Reset(PE_VGM* self, void);
	UINT8 VGMEngine_Seek(PE_VGM* self, UINT8 unit, UINT32 pos);
	UINT32 VGMEngine_Render(PE_VGM* self, UINT32 smplCnt, WAVE_32BS* data);
