...
static UINT8 device_start_k007232(const DEV_GEN_CFG* cfg, DEV_INFO* retDevInf)
{
    ...
    {
        // Initialize to center balance (both channels equal)
        chip->voice[ch].vol[0] = 128; // Left
        chip->voice[ch].vol[1] = 128; // Right
    }
...
static void k007232_write(void* chip, UINT8 offset, UINT8 data)
{
    ...
    // Handle global registers (offset 0x0C/0x0D) and off-chip controls (offset 0x0E to 0x13) first
    switch (offset)
    {
        case 0x0C:  // SLEV register - stereo volume control
        ...
        case 0x0D:
            info->loop_en = data; // Set loop enable for both channels
            return;
        // below is off-chip controls
        case 0x0E: // voice #0 sample bank
        case 0x0F: // voice #1 sample bank
            info->voice[(offset & 1)].bank = data << 17;
            return;
        case 0x10: // voice #0 Left volume
        case 0x11: // voice #0 Right volume
        case 0x12: // voice #1 Left volume
        case 0x13: // voice #1 Right volume
    ...
    // Handle per-channel registers
    KDAC_Voice* v = &info->voice[(offset >= 6) ? 1 : 0];
    UINT8 base = (offset >= 6) ? 6 : 0;
    switch (offset - base)
    {
        case 0: case 1:
            // 4 bit / 8 bit frequency mode is not implemented for now
...
static void k007232_set_mute_mask(void* chip, UINT32 MuteMask)
{
    ...
    for (UINT8 i = 0; i < KDAC_A_PCM_MAX; i++)
        info->voice[i].mute = (MuteMask >> i) & 1;
}
